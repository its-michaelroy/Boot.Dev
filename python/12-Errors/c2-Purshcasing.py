def make_purchases(purchase_orders):
    leftovers = []  # Create an empty list of leftovers

    for order in purchase_orders:
        try:
            # Try to process the order with the purchase function
            remaining_money = purchase(order["price"], order["money_available"])
            # If successful, append the remaining money to the leftovers list
            leftovers.append(remaining_money)
        except Exception as e:
            # If an exception is raised, print the exception message
            print(e)

    return leftovers  # Return the leftovers list
